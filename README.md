...
로그인 후 첫 화면으로, 활성 알림 채널 수와 최신 리콜 건수를 요약해서 보여줍니다. 알림 채널(이메일, 웹 푸시, 카카오 알림톡) 활성화 상태와 최근 리콜 현황을 한눈에 확인할 수 있습니다.

![대시보드](imgs/dashboard.png)

### 리콜 목록

식약처에서 수집된 리콜 정보를 목록으로 표시합니다. 식품명, 제조사, 회수 사유로 검색할 수 있으며 리콜 등급 및 식품 유형 태그가 함께 표시됩니다.

![리콜 목록](imgs/recall-list.png)

### 리콜 상세

개별 리콜 항목의 상세 정보 화면입니다. 회수 사유, 회수 방법, 발표일, 유통기한, 포장단위, 바코드, 업체 주소 등 식약처 원문 데이터를 모두 표시하며 식약처 원문 링크도 제공합니다.

![리콜 상세](imgs/recall-detail.png)

### 알림 설정

알림을 받을 채널(이메일, 웹 푸시, 카카오 알림톡)을 ON/OFF로 관리하는 화면입니다. 웹 푸시 및 이메일 테스트 발송 기능도 제공합니다.

![알림 설정](imgs/alim.png)

### 웹 푸시 알림

신규 리콜 발생 또는 테스트 발송 시 브라우저로 수신되는 웹 푸시 알림입니다.

![웹 푸시 알림](imgs/web-push-alim.png)
